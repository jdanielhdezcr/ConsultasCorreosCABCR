...
namespace DatosUsuarios
{
    ...
    public partial class reloaddb : System.Web.UI.Page
    {
        ...
        protected void cmd_check_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (MySqlConnection con = new MySqlConnection(cs)) //definimos una variable tipo conexion sql, que obtiene por parámetro la cadena de conexion
                {
                    ...
                    cmd.Connection = con;
                    cmd.CommandText = "DELETE FROM LAWYERS";
                    con.Open();
        ...
        public void BulkInsert(string csvfile, string table, string fieldterminator, string lineterminator, int linestoskip)
        {
            ...
            try
            {
                var bl = new MySqlBulkLoader(con)
                {
                    ...
                    FileName = csvfile,
                    Columns = { "codigo", "nombre", "apellido1", "apellido2", "tipo1", "cedula", "incorporad", "fallecido", "suspendido", "email1", "telofi1", "celular1", "sexo", "direcciofi", "telofi2", "email2", "monto", "suspenvolu", "suspenmoro" },
                };
                bl.Local = true;
            ...
            if (FileUpload1.HasFile)
            {
                ...
                for (int i = 0; i < validFileTypes.Length; i++)
                {
                    ...
                    Server.HtmlEncode(FileUpload1.FileName);
                        ...
                        BulkInsert(savePath, "abogados1.LAWYERS", ";", "\r\n", 0);

                        //ingresa un nuevo registro en el log de actualizaciones
                        string comm = "INSERT INTO UpdateLog VALUES('"+this.User.Identity.Name+ "','CIJUL',SWITCHOFFSET(GETDATE(), '-06:00'))";
                ...
                //    Label1.Text = "Archivo cargado exitósamente.";
